...
jQuery(document).on("action.ready_trx_addons", function () {
  ...
  if (jQuery(".trx_addons_login_link:not(.inited)").length > 0) {
    jQuery(".trx_addons_login_link:not(.inited)")
      .addClass("inited")
      .on("click", function (e) {
        jQuery(this)
          .find(".trx_addons_login_menu")
          .slideToggle()
          .parent()
    ...
    jQuery("body").on("click", function (e) {
      jQuery(this)
        .find(".trx_addons_login_menu:visible")
        .slideUp()
        .parent()
...
jQuery(document).on("action.init_shortcodes", function (e, container) {
      ...
      if (wheel_dir == -1) {
        ...
        setTimeout(function () {
          if (items.prev >= 0 && items.prevOffset >= scroll_offset - wh - ah)
            trx_addons_scroll_to_anchor(
              toc_menu_items.eq(items.prev).find("a"),
              false
            );
        ...
        setTimeout(function () {
          if (items.next >= 0 && items.nextOffset <= scroll_offset + wh + ah)
            trx_addons_scroll_to_anchor(
              toc_menu_items.eq(items.next).find("a"),
              false
            );
          else
            trx_addons_document_animate_to(
              Math.min(jQuery(document).height(), scroll_offset + wh)
            );
        }, 10);
      }
      if (doit) {
  ...
  function trx_addons_detect_active_toc() {
    ...
    toc_menu_items.each(function (idx) {
      ...
      var off_next =
        id_next && idx < toc_menu_items.length - 1 && jQuery(id_next).length > 0
      ...
      if (
        ...
        scroll_offset < off_next - fixed_rows_height
      ) {
  ...
  function trx_addons_scroll_to_anchor(link_obj, click_event) {
    var href = click_event
      ? link_obj.attr("href")
      : "#" + link_obj.parent().data("id");
    var pos = href.indexOf("#");
  ...
  function trx_addons_build_page_toc() {
    ...
    jQuery('[id^="toc_menu_"],.sc_anchor').each(function (idx) {
      ...
      toc +=
        ...
        icon +
        '"' +
        (sow_icon ? ' data-sow-icon="' + sow_icon + '"' : "") +
        "></a>" +
        "</div>";
    });
    if (toc_count > 0)
  ...
  function trx_addons_menu_collapse() {
    ...
    jQuery(".sc_layouts_menu").each(function () {
      ...
      if (li_collapse.length == 0) {
        ul.append(
          '<li class="menu-item menu-collapse"><a href="#" class="sf-with-ul trx_addons_icon-dot3"></a><ul class="submenu"></ul></li>'
        );
...
jQuery(document).on("action.ready_trx_addons", function () {
  ...
  jQuery(
    ...
    "click",
    "ul.product-categories.plain li a .open_child_menu",
    function (e) {
